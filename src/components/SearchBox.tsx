...
export function SearchBox() {
  ...
  return (
    <form onSubmit={handleSubmit} className="relative w-full min-w-0">
      <div className="flex min-w-0 border border-[var(--rule)]">
        <input
          type="search"
          value={query}
          onChange={handleChange}
          placeholder="Search the dictionary"
          className="min-w-0 flex-1 border-0 bg-transparent py-3 pl-4 pr-2 font-body text-[1.1rem] text-[var(--ink)] placeholder:italic placeholder:text-[var(--ink-faint)] focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[var(--blue-accent)]"
          aria-label="Search the dictionary"
          autoComplete="off"
        />
        <button
          type="submit"
          className="shrink-0 bg-[var(--ink)] px-4 py-3 font-body text-[1rem] text-white hover:bg-[var(--red-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--blue-accent)] focus:ring-offset-2 min-w-[5rem]"
        >
          Search
        </button>
      </div>
