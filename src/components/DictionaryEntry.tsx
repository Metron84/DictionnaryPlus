...
export interface VernacularNotes {
  ...
  tags?: string[];
  generation?: string;
  /** Slang etymology (string or steps). */
  etymologyJourney?: string | { steps: { era?: string; text: string }[] };
  /** Example for slang when no definition senses. */
  example?: string;
}
  ...
  return (
    <article>
      ...
      <div
        ...
        className="pt-4"
      >
        {(senses.length > 0 || vernacularNotes?.example) && (
          <>
            ...
            ) : vernacularNotes?.example ? (
              <p className="font-body text-[var(--ink)]">
                <span className="font-body text-[0.9rem] italic text-[var(--ink-muted)] example-sentence">
                  {vernacularNotes.example}
                </span>
              </p>
            ) : null}
        ...
        {(synonyms.length > 0 || antonyms.length > 0) && (
          <div className="mt-6 min-w-0">
            ...
            <p className="font-body break-words text-[0.85rem]">
      ...
      <div
        ...
        className="pt-4"
      >
        {(etymology || vernacularNotes?.etymologyJourney) ? (
          <>
            <SectionLabel>Etymology</SectionLabel>
            {etymology && (
              <p className="font-body text-[var(--ink)]">{etymology}</p>
            )}
            {vernacularNotes?.etymologyJourney && (
              <div className="mt-3 font-body text-[var(--ink)]">
                {typeof vernacularNotes.etymologyJourney === "string" ? (
                  <p>{vernacularNotes.etymologyJourney}</p>
                ) : (
                  <ul className="list-inside space-y-1">
                    {vernacularNotes.etymologyJourney.steps?.map((step, i) => (
                      <li key={i}>
                        {step.era && (
      ...
      >
        ...
        {usageNote ? (
          ...
          <p className="font-body text-[0.9rem] text-[var(--ink-faint)]">
            No usage note for this entry. Curated usage and style notes appear for words in our{" "}
            <Link href="/slang" className="text-[var(--blue-accent)] underline decoration-dotted hover:decoration-solid">
              Slang & vernacular
            </Link>{" "}
            section.
          </p>
      ...
      >
        ...
        {vernacularNotes && (vernacularNotes.culturalNote || vernacularNotes.usageNote || (vernacularNotes.tags?.length ?? 0) > 0 || vernacularNotes.generation) ? (
          ...
          <p className="font-body text-[0.9rem] text-[var(--ink-faint)]">
            No vernacular notes for this entry. Browse{" "}
            <Link href="/slang" className="text-[var(--blue-accent)] underline decoration-dotted hover:decoration-solid">
              Slang & vernacular
            </Link>{" "}
            for words with North American and Black English context.
          </p>
        )}
      </div>
    </article>
